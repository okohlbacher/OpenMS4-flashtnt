...
class FLASHTaggerAlgorithm : public DefaultParamHandler, public ProgressLogger
{
public:
  /// constructor
  ...
  */
   ...
   * @param tag_length
   */
  void fillTags(std::vector<FLASHTnTHelpers::Tag>& tags, int tag_length = 0) const;
...
private:
  ...
  */
  ...
  void constructDAG_(FLASHTnTHelpers::DAG& dag, const std::vector<double>& mzs, const std::vector<int>& scores, int length, double tol, int mode);
  ...
  void updateTagSet_(std::set<FLASHTnTHelpers::Tag>& tag_set,
  ...
  static Size find_with_X_(const std::string_view& A, const std::string& B, Size pos = 0);
  ...
  std::map<int, std::map<int, std::vector<std::string>>> edge_aa_map_;

  std::vector<FLASHTnTHelpers::Tag> tags_; // from scan to tags

  std::set<double> common_shifts_;
  std::set<double> n_term_shifts_;
  std::set<double> c_term_shifts_;

  std::map<std::string, std::set<Size>> indexed_fasta_;
