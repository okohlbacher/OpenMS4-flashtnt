...
bool FLASHTnTAlgorithm::areConsistent_(const ProteinHit& a, const ProteinHit& b, double tol)
{
  ...
  std::vector<std::string> mod_accs1, mod_accs2;
...
{
  std::map<std::string, std::vector<int>> proteoform_map;
...
void FLASHTnTAlgorithm::run(const MSExperiment& map, const std::vector<FASTAFile::FASTAEntry>& original_fasta_entry)
{
  ...
  std::map<std::string, std::vector<Size>> tag_to_protein_indices;   // tag to protein index in fasta
  std::map<std::string, std::vector<Size>> tag_to_protein_positions; // tag to protein position
  std::map<std::string, std::vector<double>> tag_to_n_flanking_masses, tag_to_c_flanking_masses; // tag to protein flanking masses
...

  {
    ...
    {
      if (fe.identifier.starts_with("DECOY")) { decoy_count++; }
      else { taget_count++; }
  ...
  std::vector<std::string> mod_strs;
  ...
  {
    nextProgress();
    const std::string& pseq = protein_seqs[i];
    const std::string& cleanseq = cleaned_protein_seqs[i];
    ac_state.setQuery(pseq);
    ac_trie.getAllHits(ac_state);

    for (const auto& h : ac_state.hits)
    {
      std::string seq = tag_seqs[h.needle_index];
    ...
    for (int k = 0; k < (keep_underdetermined_ ? 2 : 1); k++)
    {
      for (Size mod = 0; mod <= max_mod_cntr; mod++)
      {
        ...
        for (auto& hit : proteoform_hits_)
        {
          ...
          bool is_decoy = hit.getAccession().starts_with("DECOY");
...
void FLASHTnTAlgorithm::getProteoformHitsMatchedBy(const FLASHTnTHelpers::Tag& tag, std::vector<ProteinHit>& hits) const
{
...
void FLASHTnTAlgorithm::getTags(std::vector<FLASHTnTHelpers::Tag>& tags) const
{
