...
void FLASHTaggerAlgorithm::constructDAG_(FLASHTnTHelpers::DAG& dag,
    ...
    for (int g = 0; g < (max_gap_count_ > 0 ? 2 : 1); g++) // 0 for only a.a 1 for with gap.
    {
      for (int n = 0; n < (consider_diff_ion_jumps_ ? 2 : 1); n++) // 0 for all a.a 1 for diff ion type jumps. Allow only one isotope errors.
      {
        for (int current_index = start_index; current_index < end_index; current_index++)
        {
          ...
          //if (edge_aa_map_.find(end_index) == edge_aa_map_.end()) { edge_aa_map_[end_index] = std::map<int, std::vector<std::string>>(); }
          auto& e = edge_aa_map_[end_index];

          if (e.find(current_index) == e.end()) { e[current_index] = std::vector<std::string>(); }

          if (g == 0)
          {
            for (const auto& aa : aas)
            {
              auto aaStr = n == 0 ? aa.toString() : StringUtils::toLowered(aa.toString());
          ...
          else
          {
            for (const auto& gap : gaps)
            {
              ...
              for (const auto& aa : gap)
                ss << StringUtils::toLowered(aa.toString());
...
void FLASHTaggerAlgorithm::updateTagSet_(std::set<FLASHTnTHelpers::Tag>& tag_set,
                                         std::map<std::string, std::vector<FLASHTnTHelpers::Tag>>& seq_tag,
                                         ...
                                         int mode)
{
  ...
  for (int j = 1; j < (int)path.size(); j++)
  {
    ...
    if (edge_aa_map_.find(i1) != edge_aa_map_.end() && edge_aa_map_[i1].find(i2) != edge_aa_map_[i1].end())
    {
      ...
      std::vector<std::string> tmp_seqs;
  ...
  for (const auto& seq : seqs)
  {
    ...
    if (pass)
    {
      const auto direct_tag = FLASHTnTHelpers::Tag(seq, flanking_mass, -1, tag_mzs, tag_scores, scan);
    ...
    if (pass)
    {
      const auto reverse_tag = FLASHTnTHelpers::Tag(rev_seq, -1, flanking_mass, rev_tag_mzs, rev_tag_scores, scan);
...
void FLASHTaggerAlgorithm::getTags_(const std::vector<double>& mzs, const std::vector<int>& scores, int scan, double ppm, int mode)
{
  ...
  std::set<FLASHTnTHelpers::Tag> tagSet;
  std::map<std::string, std::vector<FLASHTnTHelpers::Tag>> seq_tag;
  ...
  for (int length = min_tag_length_; length <= max_tag_length_; length++)
  {
    ...
    FLASHTnTHelpers::DAG dag(_mzs.size() * (1 + max_tag_length_) * (1 + max_gap_count_) * (1 + (consider_diff_ion_jumps_ ? 1 : 0))
                                * (1 + max_path_score_ - min_path_score_));
    constructDAG_(dag, _mzs, _scores, length, ppm, mode);

    std::set<FLASHTnTHelpers::Tag> _tagSet;
  ...
  std::sort(tags_.begin(), tags_.end(), [](const FLASHTnTHelpers::Tag& a, const FLASHTnTHelpers::Tag& b) {
...
Size FLASHTaggerAlgorithm::find_with_X_(const std::string_view& A, const std::string& B, Size pos) // allow a single X. pos is in A
{
  if (A.length() <= B.length()) return std::string::npos;
                                       ...
                                       const double max_mod_mass)
{
  ...
  {
    // int mn = int(round(pg.getMonoMass())); // unused variable
...
void FLASHTaggerAlgorithm::fillTags(std::vector<FLASHTnTHelpers::Tag>& tags, int tag_length) const
{
                                                                   ...
                                                                   const FLASHTnTHelpers::Tag& tag)
{
  ...
  while (pos != std::string::npos)
  {
    ...
    if (tag.getNtermMass() > 0)
    {
      ...
      if (! nterm.empty())
      {
        if (x_pos != std::string::npos) { nterm.erase(remove(nterm.begin(), nterm.end(), 'X'), nterm.end()); }
    ...
    else
    {
      ...
      if (! cterm.empty())
      {
        if (x_pos != std::string::npos) cterm.erase(remove(cterm.begin(), cterm.end(), 'X'), cterm.end());
