#!/usr/bin/env python3
"""Build FLASHTnT against the released Core/FLASH SDKs and pinned installed CLI."""
...
def main() -> None:
    ...
    parser.add_argument("--flash-dir", required=True, type=Path,
                        help="verified and extracted FLASH backend release")
    ...
    flash = core_prefix(args.flash_dir.resolve())
    ...
    env = os.environ.copy()
    runtime_dirs = [dependency_prefix / "bin", core / "bin", cli_install / "bin", flash / "bin"]
    env["PATH"] = os.pathsep.join(str(path) for path in runtime_dirs) + os.pathsep + env["PATH"]
    library_dirs = os.pathsep.join(
        [str(dependency_prefix / "lib"), str(core / "lib"), str(cli_install / "lib"), str(flash / "lib")])
    ...
    prefixes = [core.as_posix(), cli_install.as_posix(), flash.as_posix()]
    options = []
